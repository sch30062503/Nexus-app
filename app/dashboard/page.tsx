...
export default function DashboardPage() {
  ...
  // --- REPUTATION & RESTRICTION CALCULATIONS ---
  ...
  const executePurchase = async (scope: string, price: number, targetSlug: string | null) => {
    if (targetSlug === 'lobby' || targetSlug === 'general') {
      return alert("The Lobby is a protected zone. Broadcasts are prohibited here.");
    }
        ...
        {view === 'admin' ? (
          <div className="h-full max-w-6xl mx-auto p-12 space-y-12 overflow-y-auto">
             <header className="border-b border-white/5 pb-8">
                ...
                <p className="text-[10px] text-zinc-500 mt-2 italic">Broadcast access restricted in Lobby sector.</p>
             </header>
             ...
             <section className="space-y-6">
              {!storeTarget ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <button 
                    disabled={!canBroadcastDistrict}
                    onClick={() => setStoreTarget({scope: 'district', id: '', name: ''})} 
                    className={`p-8 border text-left transition-all ${canBroadcastDistrict ? 'border-white/5 bg-zinc-900/20 hover:border-blue-500/40' : 'border-red-900/20 bg-red-900/5 opacity-50 cursor-not-allowed'}`}
                  >
                    <p className="text-[10px] font-black text-zinc-500 uppercase mb-2 flex justify-between">
                      District_Pulse {!canBroadcastDistrict && <Lock size={10} />}
                    </p>
                    <p className="text-2xl font-black text-white">-500</p>
                  </button>

                  <button 
                    disabled={!canBroadcastGlobal}
                    onClick={() => executePurchase('global', 2000, null)} 
                    className={`p-8 border text-left transition-all ${canBroadcastGlobal ? 'border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10' : 'border-red-900/20 bg-red-900/5 opacity-50 cursor-not-allowed'}`}
                  >
                    <p className="text-[10px] font-black text-emerald-500 uppercase mb-2 flex justify-between">
                      Global_Broadcast {!canBroadcastGlobal && <Lock size={10} />}
                    </p>
                        ...
                        .map(d => {
                          ...
                          return (
                            <button 
                              ...
                              className={`p-4 border text-[10px] font-black uppercase transition-all ${hasUnlocked ? 'border-white/10 bg-black hover:border-white/30 text-zinc-400' : 'border-red-900/20 bg-black text-zinc-800 cursor-not-allowed'}`}
                            >
                              {d.name} {!hasUnlocked && <Lock size={8} className="inline ml-1" />}
                            </button>
                          );
                        })}
                   </div>
              ...
              {tierBroadcast && (
                <div className="mx-8 mt-4 p-4 bg-zinc-900/80 border border-white/10 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[8px] text-emerald-500/50 mt-1 font-black uppercase">Channel_Priority</p>
                  </div>
                </div>
              )}
