...
export default function DashboardPage() {
  ...
  const loadNexus = async () => {
    ...
    if (dRes.data) {
      ...
      // If we are just starting and in chat view, enter the lobby
      const lobby = dRes.data.find(d => d.slug === 'lobby');
      if (lobby && !activeDistrict) enterRoom(lobby);
    }
    loadBroadcasts();
    setLoading(false);
  };
  ...
  const enterRoom = async (district: any) => {
    // FIX: Kill existing subscription before starting new one
  ...
  const transmitSignal = async (e: React.FormEvent) => {
    ...
    // Immediate reload of profile signal
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-[#020202] text-zinc-400 font-mono overflow-hidden">
          ...
          <div className="flex items-center gap-6 border-l border-white/10 pl-8">
            {districts.filter(d => !d.parent_slug).map(d => (
              <button 
                ...
                onClick={() => enterRoom(d)} 
                className={`text-[10px] font-black uppercase transition-all ${activeDistrict?.slug === d.slug && view === 'chat' ? 'text-emerald-400' : 'text-zinc-600 hover:text-zinc-300'}`}
              >
                {d.name}
              </button>
            ))}
      ...
      <main className="flex-1 overflow-hidden">
        {view === 'admin' ? (
          <div className="h-full max-w-6xl mx-auto p-12 overflow-y-auto space-y-12 text-center md:text-left">
              ...
              <aside className="w-64 border-r border-white/5 bg-black flex flex-col shrink-0">
                ...
                <div className="flex-1 p-4 space-y-3 overflow-y-auto">
                  {subTiers.map(tier => {
                    ...
                    return (
                      <button key={tier.slug} disabled={isLocked} onClick={() => enterRoom(tier)} className={`w-full relative flex items-center gap-4 p-3 border transition-all ${activeDistrict.slug === tier.slug ? 'border-emerald-500/50 bg-emerald-500/5 text-emerald-400' : isLocked ? 'border-white/5 opacity-40' : 'border-white/5 text-zinc-600'}`}>
                        <div className="text-left">
                          <p className="text-[10px] font-black uppercase truncate">{tier.name}</p>
                          {isLocked && <p className="text-[8px] font-bold text-zinc-500 uppercase">{progress}% Locked</p>}
                        </div>
                      </button>
                    );
                  })}
            ...
            <div className="flex-1 flex flex-col bg-black relative">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6">
                {filteredMessages.length > 0 ? filteredMessages.map((m) => (
                  ...
                  <div className="h-full flex items-center justify-center opacity-20 italic text-[10px]">No transmissions in this sector...</div>
                )}
                <div ref={scrollRef} />
              </div>
